import { raw, Express, Request, Response } from "express"
...
async function uploadPublisherLogo(req: Request, res: Response) {
	try {
		...
		let setTableObjectFileResponse =
			TableObjectsController.SetTableObjectFile({
				accessToken,
				uuid: logoUuid,
				data: req.body,
				type: contentType
		...
		if (!isSuccessStatusCode((await setTableObjectFileResponse).status)) {
			throwEndpointError(apiErrors.unexpectedError)
		}
